                ...
                <div className='gridContainer'>
                    ...
                    <div className='gridItem main_content'>
                        <div>
                            ...
                            <br />
                            <Image src={h1_tag_paired.src} alt="Paired tag Example." className="IMG" />

                            <p>There are many other paired tags such as:</p>
                            <br />
                            {<CodeBox code={paired_tag} language="Html" />}
                            <hr /><br /><br />
                            <h3>Unpaired tag</h3>
                            <br />
                            ...
                            <p>For Example:</p>
                            {<CodeBox code={Unpaired_tag} language="Html" />}
                            ...
                            {<CodeBox code={Unpaired_tag_1} language="Html" />}
                            ...
                            <hr />
                            <br /><br /><br />
                            <h3><strong>Important tags in Html</strong></h3>
                            <br />
                            <ul className='ul_tag_Privacy'>
                                ...
                                <li className='li_tag_privacy'>{"<a>"}{" tag is used to create links, you can create links within a page or globally or link a page to another page.<br/>href is teh mose important attribute in htis tag, it is used to provide the address to link a component."}<br/>{<CodeBox code={href_attribute} language="Html"/>}</li>
                                <li className='li_tag_privacy'>{"<Image>"} tag in used to insert images in a webpage.<br/>Teh src is teh attribute to provide {"image's"} address.<br/>Example:<br/>{<CodeBox code={src_attribute} language="Html"/>}<strong>You need the <i>img.png</i> in your {"website's"} root directory.<br/>The <i>img.png</i> is just a file name.</strong></li>
                                <li className='li_tag_privacy'>{"ul"} tag is used to create a unordered list. which means this is a list wiht no numbring or indexing.<br/>{"<ol>"} tag is used to creae a ordered list. which means this is a list with numbring or indexing.<br/>{"<li>"} tag is used to create list-items. It is used in both {"<ul> and <ol>"} tags.<br/><br/><strong>Example: of ul (unordered list)</strong>{<CodeBox code={ul_li} language="Html"/>}<strong>Output:</strong><br/><Image src={ul_tag.src} alt='Output example of ul tag'/><br/><strong>Example: of ol (ordered list)</strong><br/>{<CodeBox code={ol_li} language="Html"/>}<strong>Output:</strong><br/>{<Image src={ol_tag.src} alt="Output example of ol tag" />}<br/>You can clearly see there is dot in Unordered list, And <br/>Numbers in Ordered list</li>
                                <li className='li_tag_privacy'>{"<table>"} tag is used to create table in Html, {"<th>, <tr> and <td>"} is the child tags of {"<table>"} tag.<br/>{"<th>"} refers to table heading. it is used to create column in table in Html.<br/>{"<tr>"} refers of table row. it is used to create table row.<br/>{"<td>"} refers to table data. it is used to insert data in rows of the table according to the column.<br/>For example:{<CodeBox code={table_tag_e} language="Html"/>}<strong>Output:</strong><br/>{<Image src={table_tag.src} alt="table tag output" />}</li>
                                <li className='li_tag_privacy'>{"<form>"} tag is used to create form such as login form, contact form etc.<br/>{"<input>"} tag is most important and most used tag in form tag. this is used for getting information from the user and use it as we need.<br/>{"<input>"} tag is unpaired tag. Your can use it as a child tag of {"<form>"} but this is not a child of {"<form>"} tag. you can use anywhere you want such as, to create search bar.... etc.<br/>{"<button>"} is a another most important tag in Html. it is used to create button in Html, you can use it as a child component But this is not a child component of {"<form>"} tag. you can use it anywhere you want like {"<input>"} tag.<br/><strong>For Example:</strong>{<CodeBox code={form} language="Html"/>}<strong>Output:</strong><br/>{<Image src={form_tag.src} alt="form tag example" />}</li>
                                <li className='li_tag_privacy'><strong>heading tags</strong><br/>Heading tags are used to add heading in Html. There are six types of heading in Html. Every heading has different use and importance. {"<h1> <h2>.......<h6>"}<br/><strong>For Example:</strong>{<CodeBox code={heading} language="Html"/>}<strong>Output:</strong><br/>{<Image src={heading_tag.src} alt="heading tags example"/>}</li>
                            </ul>
                            <hr />
                        </div>
                    </div>
